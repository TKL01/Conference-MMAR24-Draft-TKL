...
The Robot Operating System (ROS) is one of the most popular pieces of software when it comes to creating a basis for robots and creating robot applications \cite{rosOrg}. 
Nevertheless, it is a challenge for people with little or no knowledge of robot software to familiarise themselves with this software. 
...
The application can also be used via any device with web browser support. 
The robot in question is the Husky Unmanned Ground Vehicle (UGV) from Clearpath, which is a medium-sized mobile robotics platform as shown in fig. \ref{fig:huskyClearpath}.
\begin{figure}[htbp]
...
\end{figure}
With a maximum payload of 75 kg, other robots or tools can be transported or mounted on the platform, for instance \cite{huskyClearpath}. 
...
Please number citations consecutively within brackets \cite{rosOrg}. The 
sentence punctuation follows the bracket \cite{huskyClearpath}. Refer simply to the reference 
